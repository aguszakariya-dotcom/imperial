<div class="row mt-5 justify-content-center">
    <div class="animate__animated col-xl-3 px-2 mx-3 collapse" id="kiri">
        <div class="card shadow">
            ...
            <div class="card-body ">
                <form action="<?= BASEURL; ?>/tahapan/tambahHafidz" class="form-input text-sm text-start pr-2" method="post" enctype="multipart/form-data" id="inForm">
                    <div class="mb-2 row">
                        <!-- <label class="col-sm-5 col-form-label">Saldo Sebelumnya</label> -->
                        <div class="col-sm-7">
                            <input type="text" class="form-control " name="sebelumnya" id="sebelumnya" value="<?= $data['hafidzSaldo']['saldo']; ?>" hidden="true">
                        </div>
                    </div>
                    <hr>
                    <div class="mb-2 row">
    ...
    <div class="col-lg-7">
        <div class="card">
            <div class="card-header bg-black">
                <div class="card-title text-bold" id="keKiri"><i class="fa-solid fa-comments-dollar"></i> Saldo  Rp. <?= number_format($data['hafidzSaldo']['saldo'], 0, ',', '.'); ?></div>
            </div>
            <div class="card-body">
                <table class="table table-striped">
                    ...
                    <tbody>
                        <?php 
                        $no = 1;
                        foreach ($data['hafidz'] as $hfd) :
                        ?>
                        <tr>
                            <td><?= $no++; ?></td>
                            <td><?= $hfd['tanggal']; ?></td>
                            <td><?= $hfd['keterangan']; ?></td>
                            <td class="text-danger">Rp. <?= number_format($hfd['tarik'], 0, ',', '.'); ?></td>
                            <td class="text-primary">Rp. <?= number_format($hfd['setor'], 0, ',', '.'); ?></td>
                            <td class="text-bold">Rp. <?= number_format($hfd['saldo'], 0, ',', '.'); ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
